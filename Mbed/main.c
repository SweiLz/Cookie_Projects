...
unsigned int calHz(int Hz) {
    return (unsigned int) (250000 / Hz);
}
...
void INT_EN_X() {
    input(Encode_X_B) ? Current[0]-- : Current[0]++;
}
#INT_EXT0

void INT_EN_Y() {
    input(Encode_Y_B) ? Current[1]-- : Current[1]++;
}
#INT_EXT2

void INT_EN_Z() {
    input(Encode_Z_B) ? Current[2]-- : Current[2]++;
}
...
void PID_ISR() {
    for (int i = 0; i < 3; i++) {
        ...
        Derivative[i] = Error - LastError[i];
...
void INT_Ref() {
    //    printf("%d, %d, %d", Current[0], Current[1], Current[2]);
...
int main() {
    ...
    setup_timer5(TMR_INTERNAL | TMR_DIV_BY_64, calHz(20));
